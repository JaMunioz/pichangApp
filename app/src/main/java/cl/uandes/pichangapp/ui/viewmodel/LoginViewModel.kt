...
class LoginViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun emailNotExist(email: String): Boolean {
        val localUsers = repository.getAllUsers().value
        if (localUsers!!.isEmpty()) {
            return false
        } else {
            var a = localUsers.find { it.email == email }
            if (a == null) {
                return false
            }
            return true
